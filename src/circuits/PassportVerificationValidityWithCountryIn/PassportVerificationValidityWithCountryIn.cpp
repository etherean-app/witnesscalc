...
void PassportVerificationValidityWithCountryIn_11_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void PassportVerificationValidityWithCountryIn_11_run(uint ctx_index,Circom_CalcWit* ctx);
...
PassportVerificationValidityWithCountryIn_11_run };
...
uint get_main_input_signal_no() {return 895;}

uint get_total_signal_no() {return 2201;}

uint get_number_of_components() {return 78;}

uint get_size_of_input_hashmap() {return 256;}

uint get_size_of_witness() {return 449;}
...
ctx->componentMemory[coffset].templateName = "PassportVerificationValidityWithCountryIn";
ctx->componentMemory[coffset].signalStart = soffset;
ctx->componentMemory[coffset].inputCounter = 895;
ctx->componentMemory[coffset].componentName = componentName;
ctx->componentMemory[coffset].idFather = componentFather;
...
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "nationality";
Bits2Num_9_create(csoffset,aux_cmp_num,ctx,new_cmp_name,myId);
mySubcomponents[aux_create+i] = aux_cmp_num;
csoffset += 25 ;
aux_cmp_num += 1;
...
for (uint i = 0; i < 6; i++) {
std::string new_cmp_name = "nationalityInNumbers"+ctx->generate_position_array(aux_dimensions, 1, i);
Bits2Num_9_create(csoffset,aux_cmp_num,ctx,new_cmp_name,myId);
mySubcomponents[aux_create+i] = aux_cmp_num;
csoffset += 25 ;
aux_cmp_num += 1;
...
// load src
// end load src
Fr_copyn(aux_dest,&signalValues[mySignalStart + 152],744);
}
// no need to run sub component
...
Fr_lt(&expaux[0],&lvar[2],&circuitConstants[7]); // line circom 39
while(Fr_isTrue(&expaux[0])){
{
uint cmp_index_ref = 1;
{
...
// load src
// end load src
Fr_copy(aux_dest,&signalValues[mySignalStart + ((1 * (472 + Fr_toInt(&lvar[2]))) + 152)]);
}
// run sub component if needed
if(!(ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter -= 1)){
Bits2Num_9_run(mySubcomponents[cmp_index_ref],ctx);
...
Fr_lt(&expaux[0],&lvar[3],&circuitConstants[24]); // line circom 47
while(Fr_isTrue(&expaux[0])){
{
...
Fr_lt(&expaux[0],&lvar[4],&circuitConstants[7]); // line circom 50
while(Fr_isTrue(&expaux[0])){
{
...
// load src
// end load src
Fr_copy(aux_dest,&signalValues[mySignalStart + (((24 * Fr_toInt(&lvar[3])) + (1 * Fr_toInt(&lvar[4]))) + 8)]);
}
// run sub component if needed
if(!(ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter -= 1)){
Bits2Num_9_run(mySubcomponents[cmp_index_ref],ctx);
...
PFrElement aux_dest = &lvar[4];
// load src
Fr_add(&expaux[0],&lvar[4],&circuitConstants[2]); // line circom 50
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
Fr_lt(&expaux[0],&lvar[4],&circuitConstants[7]); // line circom 50
...
PFrElement aux_dest = &lvar[2];
// load src
Fr_add(&expaux[0],&lvar[2],&ctx->signalValues[ctx->componentMemory[mySubcomponents[((1 * Fr_toInt(&lvar[3])) + 8)]].signalStart + 0]); // line circom 58
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
...
Fr_eq(&expaux[0],&lvar[2],&circuitConstants[2]); // line circom 61
...
PassportVerificationValidityWithCountryIn_11_create(1,0,ctx,"main",0);
PassportVerificationValidityWithCountryIn_11_run(0,ctx);
}
